...
namespace MedicalLibrary.Agent
{
    ...
    public class EyeKensaItemMaster
    {
        ...
        static Dictionary<string, EyeKensaItemMaster> dict = new Dictionary<string, EyeKensaItemMaster>();

        public static Dictionary<string, EyeKensaItemMaster> Dict
        {
            get
            {
                if (dict.Count == 0)
                {
                    foreach (DataRow r in EyeDict.EyeSet.Tables["KensaItem"].Rows)
                    {
                        if (r["Code"].ToString().Length > 0 && !dict.ContainsKey(r["Code"].ToString()))
                        {
        ...
        /// <summary>
        /// 該当する検査種別について、XMLで定義されている全ての検査項目マスターの辞書を返す。
        /// </summary>
        /// <param name="kensa_id"></param>
        /// <returns></returns>
        public static List<EyeKensaItemMaster> ListAllByKensaId(string kensa_id)
        {
