...
### 修正
- `CanonRKF1Form` を × ボタンで閉じたときにシリアルポートが開いたまま残る問題を修正（EyeData.exe 内でフォームを開くようになり、閉じてもプロセスが終了しなくなったため）
- `CanonRKF1Form` で一度もファイルを出力していない状態で受信エラーが起きると `NullReferenceException` になる問題を修正
